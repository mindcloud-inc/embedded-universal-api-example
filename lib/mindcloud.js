...
// Which company the API key belongs to. Always ask rather than assuming the
// first company you can see: an admin key can list every organization.
export const getMe = async (apiKey) => {
  return request({ method: 'GET', path: '/v2/me', apiKey });
};

export const getCompany = async ({ companyId, apiKey } = {}) => {
  const where = encodeURIComponent(`id=="${companyId}"`);
  return request({ method: 'GET', path: `/v2/companies?where=${where}&limit=1`, apiKey });
};
...
// Lists an installation's connections (RSQL filter over /v2/connections).
export const listInstallationConnections = async ({ installationId }) => {
  const where = encodeURIComponent(`installationId=="${installationId}"`);
  return request({ method: 'GET', path: `/v2/connections?where=${where}` });
};

// Resolves a lookup-backed action argument (e.g. a Slack channel NAME to its
// id) through the Universal API's lookup endpoint.
export const resolveUniversalLookup = async ({ appSlug, actionSlug, argumentKey, connectionId, q }) => {
  const query = `connectionId=${encodeURIComponent(connectionId)}&q=${encodeURIComponent(q)}`;
  return request({
    method: 'GET',
    path: `/v2/universal/apps/${encodeURIComponent(appSlug)}/actions/${encodeURIComponent(actionSlug)}/lookups/${encodeURIComponent(argumentKey)}?${query}`
  });
};
